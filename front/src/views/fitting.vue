<template>
	<div class="out">
		<h1>商品详情</h1>
		<div class="fittingDetail">
			<div class="left">
				<img :src="fitting.url" >
			</div>
			<div class="right">
				<h3>{{fitting.name}}</h3>
				<p>简介：{{fitting.intro}}</p>
				<div class="price">
					<p v-if="fitting.discount==1.00">单价：￥{{fitting.price}}</p>
					<p v-else>折扣价：<span>￥{{discountPrice}}</span><span>[{{fitting.discount*10}}折]</span></p>
				</div>
				<div class="count">
	...
	export default {
		...
		computed:{
			discountPrice:function(){
				let discountPrice=0;
				discountPrice=this.fitting.price*this.fitting.discount;
